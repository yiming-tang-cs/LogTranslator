...
package resources.ProjectTesting.testdir2;
...
import log_events.resources.ProjectTesting.Testdir2Namespace;
...
public class MetricsSystemImpl extends MetricsSystem implements MetricsSource {
  ...
  static final /* LogLOG=LogFactory.getLog(MetricsSystemImpl.class); */
			Testdir2Namespace LOG = LoggerFactory.getLogger(Testdir2Namespace.class, new SimpleLogger());
  ...
  public synchronized void start() {
    ...
    if (monitoring) {
      ...
      LOG.metrics_system_already_started(prefix, new MetricsException("Illegal start")).tag("methodCall").warn();
      return;
    }
  ...
  public synchronized void stop() {
    if (!monitoring && !DefaultMetricsSystem.inMiniClusterMode()) {
      ...
      LOG.metrics_system_not_yet_started(prefix, new MetricsException("Illegal stop")).tag("methodCall").warn();
      return;
    }
    if (!monitoring) {
      // in mini cluster mode
  ...
  private void initSystemMBean() {
    ...
    if (mbeanName == null) {
      mbeanName = resources.ExpectedTranslatedProject.MBeans.register(prefix, MS_CONTROL_NAME, this);
    }
  }
  ...
  public synchronized boolean shutdown() {
    ...
    if (refCount <= 0) {
      ...
      LOG.redundant_shutdown(new Throwable()).tag("methodCall").debug();
    ...
    callbacks.clear();
    if (mbeanName != null) {
      resources.ExpectedTranslatedProject.MBeans.unregister(mbeanName);
      mbeanName = null;
    }
